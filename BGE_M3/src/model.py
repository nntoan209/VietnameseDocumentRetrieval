...
class BGEM3Model(nn.Module):
    ...
    def __init__(self,
                 ...
                 negative_cross_device: bool = False,
        ...
        super(BGEM3Model, self).__init__()
        self.load_model(model_name, colbert_dim)
    ...
    def load_pooler(self, model_dir: str):
    ...
    def load_model(self, model_name, colbert_dim: int=-1):
        ...
        self.colbert_linear = nn.Linear(in_features=self.model.config.hidden_size,
                                        out_features=self.model.config.hidden_size if colbert_dim == -1 else colbert_dim)
        
        self.sparse_linear = nn.Linear(in_features=self.model.config.hidden_size,
                                       out_features=1)
    ...
    def compute_sub_batch_size(self, features):
        mapping = [(2048, 1), (1024, 2), (512, 4), (0, 8)]
    ...
    def colbert_embedding(self, last_hidden_state, mask):
        colbert_vecs = self.colbert_linear(last_hidden_state[:, 1:]) # remove [CLS] token
        colbert_vecs = colbert_vecs * mask[:, 1:][:, :, None].float() # remove [CLS] token
        return colbert_vecs
...
class BGEM3ModelAuxLoss(BGEM3Model): 
    def forward(self,
                query: Dict[str, Tensor] = None,
                passage: Dict[str, Tensor] = None,
                query_opposite: Dict[str, Tensor] = None,
        ...
        if self.enable_sub_batch:
            ...
            p_dense_vecs, p_sparse_vecs, p_colbert_vecs = self.encode(passage,
                                                                      sub_batch_size=self.compute_sub_batch_size(passage))
            q_opposite_dense_vecs, q_opposite_sparse_vecs, q_opposite_colbert_vecs = self.encode(query_opposite,
                                                                                                 sub_batch_size=self.compute_sub_batch_size(query_opposite))
        else:
            q_dense_vecs, q_sparse_vecs, q_colbert_vecs = self.encode(query)
            p_dense_vecs, p_sparse_vecs, p_colbert_vecs = self.encode(passage)
            q_opposite_dense_vecs, q_opposite_sparse_vecs, q_opposite_colbert_vecs = self.encode(query_opposite)
        ...
        if self.training:
            ...
            # dense loss
            dense_scores = self.dense_score(q_dense_vecs, p_dense_vecs)  # B, B * N
            aux_dense_scores = self.dense_score(q_dense_vecs, q_opposite_dense_vecs)
            if self.negative_cross_device:
                cross_q_dense_vecs = self._dist_gather_tensor(q_dense_vecs)
                cross_p_dense_vecs = self._dist_gather_tensor(p_dense_vecs)
                cross_q_opposite_dense_vecs = self._dist_gather_tensor(q_opposite_dense_vecs)

                cross_idxs = torch.arange(cross_q_dense_vecs.size(0), device=cross_q_dense_vecs.device, dtype=torch.long)
                cross_targets = cross_idxs * (cross_p_dense_vecs.size(0) // cross_q_dense_vecs.size(0))
                cross_aux_targets = cross_idxs
                
                cross_dense_scores = self.dense_score(cross_q_dense_vecs, cross_p_dense_vecs)
                cross_aux_dense_scores = self.dense_score(cross_q_dense_vecs, cross_q_opposite_dense_vecs)

                loss = self.compute_loss(cross_dense_scores, cross_targets) + 0.2 * self.compute_loss(cross_aux_dense_scores, cross_aux_targets)
            else:
                loss = self.compute_loss(dense_scores, targets) + 0.2 * self.compute_loss(aux_dense_scores, aux_targets)
...
class BGEM3ModelForInference(BGEM3Model):
