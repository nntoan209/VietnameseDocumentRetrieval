...
class EmbedCollatorCrossLingual(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        # select the text randomly between vietnamese and english
        query_lang = ['vi' if random.random() <= 0.5 else 'en' for _ in query]
        query = [self.merged_queries_vi[q] if query_lang[i] == 'vi' else self.merged_queries_en[q] for i, q in enumerate(query)]
        if self.aux_loss:
            query_opposite = [self.merged_queries_vi[q] if query_lang[i] == 'en' else self.merged_queries_en[q] for i, q in enumerate(query)]        
        ...
        if self.aux_loss:
            q_opposite_collated = self.tokenizer(
                query_opposite,
                # padding='max_length',     # used for adjusting the batch size in `get_file_batch_size()`
                padding=True,
                truncation=True,
        ...
        if self.aux_loss:
            return {"query": q_collated,
                    "query_opposite": q_opposite_collated,
                    "passage": d_collated,
