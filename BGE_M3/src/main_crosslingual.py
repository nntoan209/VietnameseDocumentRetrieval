...
from .model import BGEM3Model, BGEM3ModelAuxLoss
from .dataset_crosslingual import EmbedCollatorCrossLingual, LawsTrainDatasetCrossLingual
...
def main():
    ...
    if not training_args.aux_loss:
        logger.info("Training with main loss only")
        ...
        logger.info("Training with main loss and aux loss")
        model = BGEM3ModelAuxLoss(model_name=model_args.model_name_or_path,
                                normalized=training_args.normalized,
                                pooling_method=training_args.sentence_pooling_method,
                                negative_cross_device=training_args.negatives_cross_device,
                                temperature=training_args.temperature,
    ...
    print("Loading cross lingual queries and corpus")
    ...
    data_collator = EmbedCollatorCrossLingual(
        ...
        aux_loss=training_args.aux_loss,
    )
