...
from .model import BGEM3Model
from .dataset import LawsTrainDataset, LawsTrainDatasetv2, EmbedCollator
...
def main():
    ...
    if data_args.same_task_within_batch:
        if data_args.mix_data:
            train_dataset = LawsTrainDatasetv2(args=data_args, 
                                            batch_size=training_args.per_device_train_batch_size, 
                                            seed=training_args.seed, 
                                            num_processes=training_args.world_size,
                                            process_index=training_args.process_index)
        else:
            train_dataset = LawsTrainDataset(args=data_args, 
                                            batch_size=training_args.per_device_train_batch_size, 
                                            seed=training_args.seed, 
                                            num_processes=training_args.world_size,
                                            process_index=training_args.process_index)
