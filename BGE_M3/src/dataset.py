...
class LawsTrainDataset(Dataset):
    ...
    def print_batch_size(self, batch_size: int, train_group_size: int):
        length_list = ['0-512', '512-1024', '1024-2048', '2048-3072', '3072-4096', '4096-5120', '5120-6144', '6144-7168', '7168-inf']
        batch_size_dict = {
...
class LawsTrainDatasetv2(LawsTrainDataset):
    def __init__(self, args: DataArguments, batch_size: int, seed: int, process_index: int=0, num_processes: int=1):
        train_datasets = []
        each_data_inxs = []
        batch_size_inxs = []
        ...
        for data_dir in args.train_data:
            ...
            for file in os.listdir(data_dir):
                ...
                file_path = os.path.join(data_dir, file)
                data_size = file_path.split("_")[-1]
                if data_size not in dataset_by_size:
                    dataset_by_size[data_size] = [file_path]
                else:
                    dataset_by_size[data_size].append(file_path)       
        
        for data_size in dataset_by_size:
            data_files = dataset_by_size[data_size]
            ...
            # Add `parallel_` in `data_dir` to indicate that this dataset is parallel corpus
            flag = False
            print(f"loading data from {' and '.join(dataset_by_size[data_size])} ...")
            try:
                temp_dataset = datasets.load_dataset('json', data_files=data_files, split='train', cache_dir=args.cache_path, features=context_feat)
            except:
                temp_dataset = datasets.load_dataset('json', data_files=data_files, split='train', cache_dir=args.cache_path, features=context_feat_kd)
                if not args.knowledge_distillation:
                    temp_dataset = temp_dataset.remove_columns(['pos_scores', 'neg_scores'])
            ...
            if len(temp_dataset) == 0:
                ...
                small_datasets.append(temp_dataset)
                small_batch_size = min(small_batch_size, self.get_file_batch_size(data_files[0], batch_size, train_group_size=args.train_group_size))
            else:
                if args.max_example_num_per_dataset is not None and len(temp_dataset) > args.max_example_num_per_dataset:
                    temp_dataset = temp_dataset.select(
                        random.sample(list(range(len(temp_dataset))), args.max_example_num_per_dataset))
                ...
                each_data_inxs.append(np.arange(len(temp_dataset)) + cur_all_num)
                cur_all_num += len(temp_dataset)
                batch_size_inxs.append(self.get_file_batch_size(data_files[0], batch_size, train_group_size=args.train_group_size))
                pqloss_flag.append(flag)
